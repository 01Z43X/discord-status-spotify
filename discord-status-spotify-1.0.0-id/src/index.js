...
async function onLineChange(line) {
  ...
  const body = rawText.length >= 2 ? rawText : (rawText + ' ♪').trim().padEnd(2, '♪');
  const text = `🎵 ${body}`;
...
async function main() {
  ...
  if (!getToken()) {
    console.log('[Setup] Token belum ada. Membuka browser untuk pengaturan…');
    await runSetup();
    console.log('[Setup] ✓ Pengaturan selesai. Mulai memantau musik.\n');
  } else {
    console.log('   Token: OK  |  Tekan Ctrl+C untuk berhenti.\n');
  }
