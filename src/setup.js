...
<head>
  ...
  <title>Discord Lyrics Status — Pengaturan</title>
  <style>
...
<body>
  <div class="card">
    ...
    <p class="subtitle">Pengaturan awal — masukkan Discord User Token kamu untuk memulai.</p>

    <div id="main-form">
      <div class="steps">
        <strong>Cara mendapatkan User Token:</strong>
        1. Buka <code>discord.com/app</code> di browser<br>
        2. Tekan <code>F12</code> → tab <strong>Network</strong><br>
        3. Lakukan aksi apa saja (misalnya: ganti server)<br>
        4. Cari request mana saja → <strong>Request Headers</strong> → <code>Authorization</code>
      </div>

      <label for="token">Discord User Token</label>
      <div class="input-wrap">
        <input id="token" type="password" placeholder="Tempel token di sini…" autocomplete="off" spellcheck="false">
        <button class="toggle-vis" type="button" onclick="toggleVis()" title="Tampilkan/sembunyikan token">👁</button>
      </div>
      <div class="error" id="error"></div>
      <button class="save" id="save-btn" onclick="save()">Simpan dan mulai</button>
      <p class="note">Token disimpan secara lokal di perangkat kamu, tidak dikirim ke mana pun.</p>
    </div>

    <div class="success" id="success">
      <div class="success-icon">✅</div>
      <h2>Pengaturan selesai!</h2>
      <p>Aplikasi berjalan di latar belakang.<br>Kamu bisa menutup jendela ini.</p>
    </div>
  </div>
  ...
  <script>
    ...
    async function save() {
      ...
      if (!token) { errorEl.textContent = 'Silakan masukkan token.'; return; }

      saveBtn.disabled = true;
      saveBtn.textContent = 'Sedang memeriksa…';
      ...
      try {
        ...
        if (data.ok) {
          ...
          errorEl.textContent = data.error || 'Terjadi kesalahan yang tidak diketahui.';
          saveBtn.disabled = false;
          saveBtn.textContent = 'Simpan dan mulai';
        }
      } catch {
  ...
  return new Promise((resolve) => {
    ...
    app.post('/setup', async (req, res) => {
      ...
      if (!token || typeof token !== 'string' || token.length < 20) {
        return res.json({ ok: false, error: 'Token terlalu pendek atau tidak valid.' });
      }
      ...
      try {
        ...
        if (check.status === 401) {
          return res.json({ ok: false, error: 'Token tidak valid (ditolak Discord). Periksa kembali.' });
        }
        if (!check.ok) {
          return res.json({ ok: false, error: `Discord mengembalikan error ${check.status}. Coba lagi nanti.` });
        }
        const user = await check.json();
        console.log(`[Setup] Token valid — masuk sebagai: ${user.username}`);
      } catch {
        return res.json({ ok: false, error: 'Tidak dapat terhubung ke Discord. Periksa koneksi internet.' });
      }
    ...
    server = app.listen(0, '127.0.0.1', () => {
      ...
      console.log(`[Setup] Membuka browser di ${url}`);
